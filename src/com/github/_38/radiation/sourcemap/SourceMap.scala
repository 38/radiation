...
import com.github._38.radiation.source.{Location, InSource, NotInSource}
...
object VLQSourceMap {
    ...
    case class  SourceMap(val file:Int, 
                          val name:Int, 
                          val target:InSource, 
                          val source:InSource) extends Token with Ordered[SourceMap] {
        private def _get(idx:Int) = idx match {
            case 0 => Some(target.line)
            case 1 => Some(target.column)
            case 2 => Some(file)
            case 3 => Some(source.line)
            case 4 => Some(source.column)
            case 5 => Some(name)
            case _ => None
        }
        @tailrec
        private def _compareImpl(that:SourceMap, how:Int):Int = (this _get how, that _get how) match {
        ...
        def -(that:CodeMap) = CodeMap(file - that.file, name - that.name, target - that.target, source - that.source)

        private def _toVLQ(fid:Int = 1) = _get(fid) match {
            case Some(x) => {
